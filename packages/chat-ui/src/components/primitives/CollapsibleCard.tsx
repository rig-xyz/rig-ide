...
import { collapsibleCard, collapsibleCardFlat, collapsibleCardVars } from './collapsible-card.css';
...
export type CollapsibleCardProps = {
  ...
  /**
   * `'card'` (default, unchanged): bordered shell + header separator —
   * file-edit summaries only, per design-system Rule 9. `'line'`: no
   * border/radius/separator anywhere — a quiet work line whose expanded
   * body is an inset content block, not a second card.
   */
  chrome?: 'card' | 'line';
};
...
export function CollapsibleCard(props: CollapsibleCardProps) {
  ...
  const flat = () => props.chrome === 'line';

  return (
    <div
      class={flat() ? collapsibleCardFlat : collapsibleCard}
