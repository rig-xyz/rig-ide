...
/**
 * Disk plumbing for one open doc: initial read, debounced autosave, and live
 * absorption of edits made by anything else (agents, git, another editor).
 *
 * Ported from emdash's `doc-file-sync.ts` — the crown jewel the P0 spec calls
 * out to port faithfully. The autosave/absorb/own-write-suppression logic
 * below is unchanged; only the IO layer is swapped: emdash reads and writes
 * through `rpc.workspace.files`, keyed by an emdash project/workspace pair
 ...
 * change notification we re-read the file and drop it when its signature
 * matches — no event ordering assumptions, and correct for atomic saves that
 * arrive as create/delete pairs rather than a single 'update'.
 */
...
function describeError(error: { message?: string; kind?: string }): string {
  return error.message ?? error.kind ?? 'Unknown error';
}
...
export class DocTabResource {
  ...
  /** Writes pending edits immediately. Called on Cmd+S and on close. */
