...
class AppService implements IInitializable, IDisposable {
  ...
  /**
   * Round (Dylan's feedback — "Change hangs, native dialog never appears or
   * blocks"): this previously did `dialog.showOpenDialog(getMainWindow()!,
   * ...)` with no try/catch — a real, non-null-assertion-only bug: the `!`
   * silences TypeScript but does nothing at runtime, so if `getMainWindow()`
   * ever returned `null` at the exact moment this ran, `showOpenDialog`
   * would have been called with a literal `null` as the parent window
   * (rather than omitted, Electron's own "no parent" form), and any
   * resulting failure would reject with nothing catching it — an unhandled
   * rejection in the renderer, invisible to the user, indistinguishable
   * from a hang. Hardened on both counts: omit the window argument entirely
   * when there isn't one (Electron's documented no-parent call), and never
   * let a failure here propagate as a silent, uncaught rejection.
   */
  async openSelectDirectoryDialog(args: {
    title: string;
    message: string;
    /**
     * A starting suggestion only — the dialog is still the one place the
     * user actually decides (correction round: a path shown as running
     * TEXT elsewhere, before anyone picks it, reads as an existing fact,
     * not a suggestion — this is the honest place for one). `~/`-prefixed
     * paths are expanded (string-only, no existence check — the whole
     * point is this may not exist yet, e.g. a fresh "Download" target).
     */
    defaultPath?: string;
    /**
     * `Date.now()` at the renderer's click, when the caller wants the
     * click→native-panel latency attributed (picker-latency round: the
     * handler itself does no fs/env work, so the log below splits the
     * remaining suspects — IPC/main-loop queueing vs the OS panel itself).
     */
    requestedAt?: number;
  }): Promise<string | undefined> {
    const handlerEntry = Date.now();
    const options: Electron.OpenDialogOptions = {
      title: args.title,
      // `createDirectory`: the OS dialog's own "New Folder" button — lets
      // a "Download" destination be created right there, still entirely
      // the user's own choice of name and location.
      properties: ['openDirectory', 'createDirectory'],
      message: args.message,
      defaultPath: args.defaultPath ? expandAbsoluteOrTildePath(args.defaultPath) : undefined,
    };
    try {
      const win = getMainWindow();
      // Everything before `showOpenDialog` is string assembly — `ipcDelayMs`
      // (renderer click → this handler running) is the only app-controlled
      // span; the rest of any perceived slowness is Electron/macOS panel
      // time (notably the once-per-run cold start of the open-panel
      // service), which the resolve log below brackets.
      log.info('openSelectDirectoryDialog: showing', {
        ipcDelayMs: args.requestedAt ? handlerEntry - args.requestedAt : null,
        prepMs: Date.now() - handlerEntry,
      });
      const shownAt = Date.now();
      const result = win
        ? await dialog.showOpenDialog(win, options)
        : await dialog.showOpenDialog(options);
      log.info('openSelectDirectoryDialog: resolved', {
        openMs: Date.now() - shownAt,
        canceled: result.canceled,
      });
      if (result.canceled) return undefined;
      return result.filePaths[0];
    } catch (error) {
      // Logged, then RE-thrown — swallowing to `undefined` would make a
      // genuine failure indistinguishable from "the user cancelled," which
      // is its own kind of silent. Rethrowing lets it cross the IPC
      // boundary as a rejection a caller can catch and show, same as any
      // other RPC failure in this app.
      log.warn('App service: openSelectDirectoryDialog failed', { error: String(error) });
      throw error;
    }
  }

  /** The doc-import flow's .docx picker — same hardened window/rethrow pattern as the directory dialog above. */
  async openSelectDocumentFileDialog(args: {
    title: string;
    message: string;
  }): Promise<string | undefined> {
    const options: Electron.OpenDialogOptions = {
    ...
    try {
      ...
      log.warn('App service: openSelectDocumentFileDialog failed', { error: String(error) });
      throw error;
    }
  }

  /** The Add menu's "From file…" picker — no extension filter (a .docx routes through the import pipeline, anything else is copied in as-is), same hardened window/rethrow pattern as the pickers above. */
  async openSelectAnyFileDialog(args: {
    title: string;
    message: string;
  }): Promise<string | undefined> {
    const options: Electron.OpenDialogOptions = {
    ...
    try {
      ...
      log.warn('App service: openSelectAnyFileDialog failed', { error: String(error) });
      throw error;
    }
  }
