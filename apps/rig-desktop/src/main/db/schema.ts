...
// ── rig_* tables ─────────────────────────────────────────────────────────────
//
// New state for this app's own features (persistence-design.md, Round A) —
// deliberately separate from every table above, which is inherited Emdash
// schema kept dormant (design doc principle 3: "our tables, not Emdash's").
// Timestamps are INTEGER epoch-ms (`Date.now()`), matching this schema's
// newer tables (automations, kv, providerAccounts) rather than the older
// TEXT-CURRENT_TIMESTAMP convention above.

export const rigRigs = sqliteTable(
  'rig_rigs',
  {
    id: text('id').primaryKey(),
    /** Local filesystem path at last open — best-effort, not the identity (bindingId is). */
    path: text('path').notNull(),
  ...
  (table) => ({
    bindingIdIdx: uniqueIndex('idx_rig_rigs_binding_id').on(table.bindingId),
    lastOpenedIdx: index('idx_rig_rigs_last_opened').on(table.lastOpenedAt),
  })
);

export const rigSessions = sqliteTable(
  'rig_sessions',
  {
    /** Matches the ACP conversationId — see rig-chat-store.ts. */
    id: text('id').primaryKey(),
    rigId: text('rig_id')
      .notNull()
      .references(() => rigRigs.id, { onDelete: 'cascade' }),
    providerId: text('provider_id').notNull(),
    title: text('title'),
    /**
     * Round S2+ (session title editing): 'auto' — the write-path's own
     * first-prompt derivation (`shouldPersistTitle`) is free to keep
     * updating `title`; 'manual' — the user renamed the tab, and the
     * auto-titling write path must never overwrite it again (enforced
     * server-side too — see `main/rig/sessions.ts`'s `setTitle`, which
     * only writes `WHERE title_source != 'manual'`).
     */
     ...
     * response), distinct from `id` (this app's conversationId, the wire
     * live-model key) — null until the runtime returns one. Round B: the
     * value a genuine `loadSession`/resume needs; see
     * `renderer/lib/acp/acp-live-session.ts`.
     */
    acpSessionId: text('acp_session_id'),
  },
  (table) => ({
    rigIdIdx: index('idx_rig_sessions_rig_id').on(table.rigId),
  })
);

export const rigSessionEvents = sqliteTable(
  'rig_session_events',
  {
    id: integer('id').primaryKey({ autoIncrement: true }),
    sessionId: text('session_id')
      .notNull()
      .references(() => rigSessions.id, { onDelete: 'cascade' }),
    seq: integer('seq').notNull(),
    /** Stamped at write time in main — the honest timestamp source (persistence-design.md). */
    at: integer('at').notNull(),
    eventJson: text('event_json').notNull(),
  },
  (table) => ({
    sessionSeqIdx: uniqueIndex('idx_rig_session_events_session_seq').on(
      table.sessionId,
      table.seq
    ),
  })
...
export const rigProfiles = sqliteTable('rig_profiles', {
  userId: text('user_id').primaryKey(),
  name: text('name'),
  avatarUrl: text('avatar_url'),
  email: text('email'),
...
export const rigCommentsCache = sqliteTable(
  'rig_comments_cache',
  {
    bindingId: text('binding_id').notNull(),
...
export type RigSessionInsert = typeof rigSessions.$inferInsert;
export type RigSessionEventRow = typeof rigSessionEvents.$inferSelect;
export type RigSessionEventInsert = typeof rigSessionEvents.$inferInsert;
export type RigProfileRow = typeof rigProfiles.$inferSelect;
